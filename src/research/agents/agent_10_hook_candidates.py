...
Pure synthesis agent. Walks prior `AgentResult`s emitted by agents 1–6
and turns the freshest, dated, sourced trigger events into 2–3 cold-
outreach opener angles ("hooks") plus a single "Why Now" urgency frame.
...
This agent makes NO external calls. It is intentionally deterministic
and explainable — Phase 4 may drop in an LLM refinement pass, but the
hook ranking heuristic stays the same so the trace is reproducible.
...
async def run(ctx: AgentContext) -> AgentResult:
    ...
    Behavior:
      ...
      4. Pick the top 2–3, deduplicating by source_url. Emit one PUBLIC
         hook per pick. Pad remaining slots with NOT_FOUND "HOOK TBD"
         claims so the rendered output always shows 3 candidate slots.
      5. Emit one Why Now claim:
         - INFERRED when at least one dated trigger exists; framed off
           the highest-priority + freshest claim.
         - NOT_FOUND when no dated trigger exists.
    ...
    try:
        ...
        for parent_result, claim, _section_prio, _claim_date in picks:
            claims.append(_build_hook_claim(parent_result, claim))
        ...
        # Why Now — single sentence framing off the strongest ranked
        # claim. If no ranked claims exist we emit NOT_FOUND instead.
        if picks:
            claims.append(_build_why_now_claim(picks))
        else:
            claims.append(
                Claim(
                    text=(
...
def _build_hook_claim(parent: AgentResult, claim: Claim) -> Claim:
    """Build a PUBLIC hook claim anchored to a single dated source.

    Format: "Hook: {one-line opener angle referencing the signal}". The
    parent section title is appended so the rep can see which research
    bucket the hook came from without re-reading the full report.
    """
    snippet = _summarize_claim(claim.text or "")
    section = (parent.section_title or "").strip() or "upstream signal"
    text = f"Hook: {snippet} (anchor: {section})"
    return Claim(
...
def _build_why_now_claim(picks: List[RankedClaim]) -> Claim:
    """Synthesize a 2–3 sentence Why Now framing.

    Strategy:
      - Trigger sentence anchors on the highest-priority + freshest
        pick (already first in `picks`).
      - If a second pick exists in a DIFFERENT section, add a
        compounding sentence ("Combined with X, this is an active
        automation window.") to signal urgency.
      - `inference_logic` lists which section names contributed so the
        rep can trace the framing.
    """
    ...
    sentences = [
        ...
        (
            f"This is a fresh, dated signal — they are actively shopping "
    ...
    # Optional second sentence — compounding signal from a different
    # section bucket strengthens the urgency frame.
